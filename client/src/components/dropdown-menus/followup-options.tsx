...
import { deleteFollowUp } from '@/services/followup-service/followup-service'
...
export default function FollowUpOptions({
  ...
  const onDelete = async () => {
    ...
    if (pathname === `/follow-ups/${followUp.id}`) {
      router.back()
    } else {
    ...
    <>
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="min-w-48">
          <DropdownMenuLabel>Ações do acompanhamento</DropdownMenuLabel>
          <DropdownMenuSeparator />
          ...
          {showViewItem && (
            <DropdownMenuItem asChild>
              ...
              >
                <Eye size={16} />
                <span>
                  {useLongLabel ? 'Visualizar acompanhamento' : 'Visualizar'}
                </span>
              </Link>
            </DropdownMenuItem>
          )}
          ...
          <DropdownMenuItem asChild>
            <Link
              href={`/follow-ups/edit/${followUp.id}`}
              onClick={(event) => event.stopPropagation()}
              className="gap-2"
            >
          ...
          >
            <TrashSimple size={16} />
            <span>{useLongLabel ? 'Excluir acompanhamento' : 'Excluir'}</span>
          </DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>

      <AlertDialog {...deleteDialog.props}>
        <DeleteDialog onConfirm={onDelete} variant="followup" />
      </AlertDialog>
    </>
  )
}
